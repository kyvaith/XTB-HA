...
If XTB sends an OTP after the password step, Home Assistant shows a second form for that one-time code. The OTP is not stored. If the OTP challenge expires before the code is submitted, the integration starts a fresh login attempt only after you submit the expired form again. Background polling and automatic reauth do not start a new password login by themselves, so the bridge should not generate OTP SMS messages while you are away. Before a cached TGT expires, the bridge tries to reconnect active xStation clients using the still-valid TGT so polling can continue on a fresh WebSocket without asking for OTP. If a WebSocket dies after the cached TGT has already expired, the bridge returns a reauthentication-required response instead of letting the underlying xStation client start its own CAS reconnect loop. After a successful OTP login, the bridge keeps persistent Playwright browser profiles under the add-on `/data` directory; reauthentication for an existing HA entry is scoped to that account number so multiple accounts do not share the same trusted-browser profile. Retirement account data, such as IKZE, is fetched through a separate XTB endpoint and the last successful value is cached for up to 7 days. If that endpoint cannot refresh while the trading WebSocket is still alive, the bridge keeps the cached IKZE value and marks it with `retirement_data_stale`.
